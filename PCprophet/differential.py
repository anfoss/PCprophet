...
class BayesMANOVA:
    ...
    def mrgllh(self, lbls , vals, m=None):
        ...
        for cg in grps:
            ...
            ntau=sum(lbls==cg)
            alln.append(ntau)
            cxi=(self.gam*m+np.sum(vals[lbls==cg,:], axis=0))/np.sqrt(self.gam+ntau) # correction!!
        ...
        for d in range(nin):
            # calculate h_ht
            h_ht[d]=h_ht[d]+0.5*(ngroups*self.gam*m[d]**2+sum(vals[:,d]**2)-sum(allxi[:,d]**2))

        # we may now express the loig marginal likelihood:
        lgmrgllh=nin*(self.g*np.log(self.h)-spc.gammaln(self.g))
        lgmrgllh=lgmrgllh+0.5*(nin*ngroups *np.log(self.gam) - nin*nsampls*np.log(2*np.pi))
        lgmrgllh=lgmrgllh+np.sum(-0.5*nin*(self.gam+alln))+np.sum(spc.gammaln(g_ht)-g_ht*np.log(h_ht))
        return lgmrgllh
    ...
    def mrgllh4ids(self, ids2dat, m=None, PI1=0.5):
        ...
        for cid in ids2dat.keys():
            ...
            y=ids2dat[cid].y
            if type(self).yok(y):     # this call allows overriding yok() by deriving from BayesMANOVA
                ...
                resdict["PB4DEX"].append(1/(1+np.exp(lgmrgllhn+np.log(1-PI1)-lgmrgllha-np.log(PI1))))
            else:
    ...
    def mrgllh4pcplx(self, cplx2pids, pids2dat, m=None, PI1=0.5):
        ...
        if self.modeltype=="naive":
            ...
            for cpidx in cplx2pids.keys():
                ...
                alllmla=[val for val in alllmla if val is not None and not np.isnan(val)]
                ...
                alllmln=[val for val in alllmln if val is not None and not np.isnan(val)]
...
# data input
def prepcplxdata(dfrm, pidcol, cplxcol, trgcol, valcols,
                 dologtrans=True, minval=10**-17, trg2indmap=True):
    """
...
def score_complexes(dfrm):
    ...
    (cplx2pids, pids, Xdfrm, pids2dat)=prepcplxdata(dfrm, pidcol="ID",
                                                          cplxcol="CMPLX", trgcol="COND",
                                                          valcols=valcols)
...
def create_complex_report(infile, sto, sid, outfile='ComplexReport.txt'):
    ...
    com = combined.groupby(['CMPLX', 'COND'], as_index=False).mean()
    ...
    header = ['ComplexID', 'Condition', 'Members',
              'Stoichiometry', '# Members', 'Replicate', 'Apex Peak',
              'Prediction confidence', 'Completness', 'GO Score', 'Is Complex',
              'Reported', 'Estimated MW']
...
def create_ppi_report(infile='ComplexReport.txt', outfile='PPIReport.txt'):
    ...
    header = ['ComplexID', 'ProteinA', 'ProteinB', 'StoichiometryA',
              'StoichiometryB', 'Condition', 'Replicate', 'Reported'
             ]
    io.wrout(outf, outfile, header)


def runner(infile, sample, outf, temp,
           score_missing, weight_pre, thresh, weights):
    """
    args
    0 combined_file,
    1 outfolder,
    ...
    combined = pd.read_csv(infile, sep="\t")
    # explode combined INT into multiple cols
    valcols = list(map("{0}".format, list(range(1,73))))
    combined[valcols] = combined.INT.str.split("#",expand=True)
    combined[valcols] = combined[valcols].apply(pd.to_numeric, errors='coerce')

    allprot, allcmplx = [], []
    for cnd in ids.keys():
        if cnd != 'Ctrl':
            tmp = combined[combined['COND'].isin(['Ctrl', cnd])]
            prot, cmplx = score_complexes(tmp)
    ...
    allprot.to_csv(os.path.join(outf, 'DifferentialProteinReport.txt'), sep="\t")
    allcmplx.to_csv(os.path.join(outf, 'DifferentialComplexReport.txt'), sep="\t")
