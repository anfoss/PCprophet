...
def plot_positive(out_fold, tmp_fold, sid):
    ...
    xx = lambda row: np.array(sig.resize_plot(
                                              row['INT'],
                                              input_fr = 72,
                                              output_fr = 72
                                              ))
...
def plot_sec_diff(comb, ids, positive, out_fold):
    ...
    mm = comb.groupby(groups, as_index=False).INT.apply(lambda g: np.mean(g.values.tolist(), axis=0)).reset_index()
